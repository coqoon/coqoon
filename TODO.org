...
* Coq side
** string literal "foo" is printed as foo, while it should be "foo" (or ("foo":expr))
** find disjoint definition names (newR, newM,...)
** constructor might have body, emit that!
...
** operator translation (x += y --> x = x + y) (Composite example)
