...
trait JavaAST extends JavaParser { // with CoqOutputter {
  ...
  def parse(r: Reader[Char], name : String) : Pair[String,Pair[Pair[Int, Int], List[Pair[Pair[Pair[String, Pair[Position,List[Position]]],Pair[Int,List[Pair[Int,Int]]]], Pair[List[Position],List[Int]]]]]] = {
