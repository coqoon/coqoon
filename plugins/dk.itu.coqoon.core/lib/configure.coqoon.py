...
parser = ArgumentParser(
    description = """\
Generate a site-specific Makefile to compile this Coqoon project and a
_CoqProject file to use it with other tools.""")
parser.add_argument(
    "vars",
    metavar = "NAME=VALUE",
...
parser.add_argument(
    ...
    default = "_CoqProject",
    dest = "project_file",
    help = "override the name of the output _CoqProject file")
parser.add_argument(
    "-s", "--strict",
    action = "store_false",
    dest = "persevere",
...
def examine_makefile(f):
    _vars = {}
    with io.open(f, mode = "r", encoding = "utf_8") as file:
        valid = False
        reading_vars = False
        for line in file:
            if not valid:
...
if os.path.isfile(args.file):
    ...
    if not valid:
        err("""\
the output Makefile already exists and was not generated by this script, \
aborting""")
    else:
...
# Similarly, we shouldn't override _CoqProject files we didn't create
if os.path.isfile(args.project_file):
    with io.open(args.project_file, mode = "r", encoding = "utf_8") as file:
        if not "Generated by configure.coqoon" in file.readline():
            err("""\
the output _CoqProject file already exists and was not generated by this \
script, aborting""")

def prompt_for(vns, prompt, default = None, checker = None):
    for vn in vns:
        if vn in variables:
            return variables[vn]
    if not args.prompt:
        ...
        if len(val) > 0:
            if checker == None:
                ...
                if status == None:
                    ...
                    warn(status)
                    return prompt_for(vns, prompt, default, checker)
    except EOFError:
        pass
    return default
...
Specify the path to the \"%s\" library.""" % elp_name, lp[1])
                    ...
                    if not os.path.isdir(lp[1]):
                        warn("""\
the library "%s" (%s) could not be found; dependencies on it will not be \
resolved correctly""" % (elp_name, vns[0]))
                        doomed = True
            pv += 1
    else:
...
try:
    with io.open(args.project_file, mode = "w", encoding = "utf_8") as file:
        file.write(u"""\
# Generated by configure.coqoon v%d on "%s"
# at %s.
#
# This _CoqProject file was automatically generated; any local changes you may
# make will not be preserved when it is next regenerated.
